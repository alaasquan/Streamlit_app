...
# Sidebar for spatial filtering options
    spatial_filter_option = st.sidebar.radio('Select Spatial Filter Option', ['Point Buffer', 'Polygon Filter'], key="spatial_filter_option")
...
# Filter based on point buffer or drawn polygon
    ...
    elif spatial_filter_option == 'Polygon Filter':
    # Allow the user to input polygon coordinates
        st.sidebar.markdown("### Input Polygon Coordinates")
        coordinates_input = st.sidebar.text_area("Enter polygon coordinates (comma-separated)", key="coordinates_input")

        # Convert the input coordinates to a Shapely geometry (assuming WGS84 coordinates)
        if coordinates_input:
            coords = [tuple(map(float, coord.split(','))) for coord in coordinates_input.split('\n') if coord]
...
# Convert the 'geometry' column to WKT (Well-Known Text) for display
    filtered_data_display = filtered_data.copy()
    filtered_data_display['geometry'] = filtered_data_display['geometry'].apply(lambda geom: geom.wkt)
